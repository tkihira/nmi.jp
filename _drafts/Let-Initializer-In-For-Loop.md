...
変数 a も i も、大外の関数内で定義されています。そして大外の関数は 1 回起動されているだけですので、変数のインスタンスも 1 個しか存在しません。よって、console.log を呼び出している関数の中から参照される i は、大外の関数内にある i を指し示しており、ループが終わった後では i は 10 になっているため、結果として `a[3]();` を呼び出しても 10 が表示されます。
...
新しい変数 `_i` のスコープを新設するために、新しい関数を作ります。そしてその関数を即時実行し、`_i` に現在の `i` の値をコピーします。そして返り値としてまた新しい関数を用意して返すのですが、その関数で参照している変数はコピーされた `_i` であり、この `_i` は初期化時に `i` が設定されて以降一切変更されないので、結果として `a[3]();` を呼び出した時には 3 が表示されます。
...
この挙動は理解さえしてしまえば便利ではあるのですが、本来のクロージャーのあるべき挙動とは異なるために、クロージャーを理解していればいるほど例外を知らないと面食らうかと思います。そしてまた、この挙動をカジュアルに利用したプログラムも多く存在するので、**JavaScript において知っておくべき例外的な挙動だと思います**。
...
これの `for ( LexicalDeclaration Expressionopt ; Expressionopt ) Statement` の部分が、今回の処理に当たります。 `LexicalDeclaration` には let もしくは const で変数が宣言されます。では処理を見てみましょう。
...
まず (1) で `oldEnv` に現在のレキシカル環境を退避して、(2) で新しいレキシカル環境 `loopEnv` を `oldEnv` を元に作ります（レキシカル環境は外側のレキシカル環境を保持します）。そして (3) で `isConst` に `LexicalDeclaration` が const であるかどうか、すなわち `for(const ...;;)` であるか `for(let ...;;)` であるかのフラグをセットします。(4) で `boundNames` という変数に宣言された変数名のリストを保存しておきます。そして (5) で `loopEnv` にその変数名を追加します。(6) で現在のレキシカル環境を、その `loopEnv` にセットします。
