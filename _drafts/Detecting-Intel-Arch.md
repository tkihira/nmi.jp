...
今回重要なのは、NaN にはビット表現において正負の違いを持てる、ということです。

### Intel アーキテクチャの特殊挙動

さて、では具体的に上記コードでなぜ Intel アーキテクチャを判定出来るのかを解説します。Intel のページにある次のシートを見てみてください。
...
この中の R Ind こと「Real Indefinite」が今回のポイントです。このシートでは、Intel の演算ではいくつかのパターンにおいて Real Indefinite になることが定義されており、そのパターンになった場合は特殊な値（R Ind）を取ることが示されています。その R Ind の表現が、ちょうど負の NaN の値になっており、上記のコードではその部分を判定して Intel アーキテクチャと判断しております。
...
なので、次のコードによって、Intel アーキテクチャかつ V8 エンジンの場合限定ですが、自分自身が JIT 化されているかどうかを確認することが出来ます。
...
まず、このコードは Intel 以外の環境だと、全て 127 を返します。手元の Intel 環境で試したところ、
