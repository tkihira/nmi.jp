...
なお本記事では Promise の `rejected` の状態について解説しておりません。基本を理解したら、別記事でぜひ学んでみてください。
...
さて普通に考えると、次は Promise がどのような状態なのかをチェックして、正常に終了しているなら（ `fulfilled` なら）データを取得したい、と考えると思います。しかし Promise は、外部から状態を確認することが出来ません（正確に言うと方法があるので余談で紹介します）。

基本的に、Promise オブジェクトを直接操作することは出来ないと考えてください。


# `then` について簡単に説明

ではどのようにして `fetch` の結果を利用するのでしょうか？そこで過去に利用されていたのが `then` です。
...
また、`f` の返り値が Promise の場合、 `then` はその Promise の関数が終わるまで次の関数を呼びます。
...
率直に行って、それ以前の callback hell と呼ばれる状態よりはマシとはいえ、 `then` は読みにくいです。なので、今は `then` を使うべきではありません。

代わりに、今は `async` と `await` があります。こちらを使いましょう。

# `async` と `await` で書き直す

ではまず、上記のコードを `async` / `await` で書きなおしてみます。
...
`async` は一般的に「エイシンク」と読まれます（英語ネイティブでも一部の人は「アシンク」と読みます）。

`async` は関数宣言の時につける修飾子で、これがつくとその関数は `async` 関数になります。

`async` 関数は、関数内部で何をやろうとも、必ず Promise を返します。例外はありません。
...
さて、なんのために `async` 関数を用意するかというと、次に紹介する `await` は `async` 関数の中でしか使えないためです。
...
`await` の効果を説明します。 `await` は、そこで一旦処理を打ち切り、引数にある Promise 内にある関数が終了するまで待ちます。Promise 内の関数が終了したり、Promise 内の関数が既に終了していた場合は、その Promise オブジェクトから値を取り出し、それを返します。引数に Promise 以外の値が指定されていた場合は、それをそのまま返し、そして処理を再開します。
...
`await` の処理はこれだけです。しかし、 `await` と `async` のおかげで、ぐっと処理が見やすくなりました。
...
今回の記事では、わかりやすさを優先するために `rejected` の状態や `thenable` のオブジェクトなどの話を意図的に省いています。 `thenable` はともかく `rejected` は必須の知識となりますので、もし自信のない方は別に調べてみてください。

乱暴に言ってしまうと、`await` `async` は非同期処理の可読性を大きくあげるための記法で、 `then` は内部的にそれを支える仕様です。もちろん `then` をしっかり理解していることはとても大切ですが、あくまでも内部仕様としての知識に留め、実際のコードで `then` を書かないようにしましょう。

ただ、 `then` を使った方が綺麗にかける状況もありますので、そういう場合は固執しないようにしましょう
...
この用例は [@hiroppy](https://twitter.com/about_hiroppy) さんに教えて頂きました。ありがとうございました！
...
そして 1 秒間経つと、ここで改めて resolve 関数が呼ばれ、 `await` から処理が再開します。最後に `4` が表示されるわけです。

すなわち答えは、1→2→3→5→4 となります！

特に `async` 関数が実行されたら即座に処理が打ち切られてコンテキストが変わる、と誤解している方が多いのですが、 `await` が登場するまでは普通の関数と同じように実行されます。逆に `await` が登場したら、そこで必ず処理が止まります。例えば、
...
これの実行結果は `1→2→4→3` となります。 `await 3` と意味のない `await` を呼んだとしても、 `async` 中に `await` が登場すると必ずそこで処理が一旦止まります。処理が止まるタイミングは `async` ではなくて `await` であるということを意識しておきましょう。
...
なお、この `promiseState` は、個人的には `then` を使ったオリジナルの書き方の方がすっきりして見やすいと思います。そういう時は `then` を使いましょう。